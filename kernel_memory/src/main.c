...
void atender_cpu_copy_mem(t_list* data, int cpu_fd){
    ...
    t_evt* evt_read = iniciar_evt_read(pid, base_origen, tamanio, cpu_fd);
    t_data_read* data_read = (t_data_read*)evt_read->data;
    agregar_evt_a_stick(evt_read);

    //sem_wait(&evt_read->s_fin);

    t_evt* evt_write = iniciar_evt_write(pid, base_destino, tamanio, data_read->datos_leidos, cpu_fd);
